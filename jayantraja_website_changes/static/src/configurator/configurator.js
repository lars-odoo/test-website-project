...
class WelcomeScreen extends Component {
    ...
    skipButton() {
        skipConfigurator(Component.env.services);
    }
}

Object.assign(WelcomeScreen, {
    components: {SkipButton},
    template: 'website.Configurator.WelcomeScreen.inherit.jayantraja',
});
...
class PaletteSelectionScreen extends Component {
    ...
    selectPalette(paletteName) {
        ...
        // window.location = '/web#action=website.theme_install_kanban_action'; // NORMAL THEME SELECTOR
        
        // RECOMMENDED THEMES SELECTOR        
        ...
        if (!themes.length) {
            console.log('=> 5');
            applyConfigurator(this, 'theme_default');
        } else {
            console.log('=> 6');
...
async function makeEnvironment() {
    ...
    const jayantrajaTemplates = await owl.utils.loadFile('/jayantraja_website_changes/static/src/configurator/configurator.xml')
    qweb.addTemplates(loaderTemplate);
    qweb.addTemplates(configuratorTemplates);
    qweb.addTemplates(jayantrajaTemplates);
